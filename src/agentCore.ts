...
const memoListPattern = /(\u67e5\u770b|\u5217\u51fa|\u770b\u770b).*(\u5907\u5fd8|\u8bb0\u5f55)|^\s*(\u6211\u7684)?\u5907\u5fd8(\u5f55)?\s*$/;
const memoSearchPattern = /(\u4e4b\u524d|\u4ee5\u524d|\u8bb0\u5f97).*(\u54ea|\u4ec0\u4e48|\u653e|\u662f)|(\u67e5\u627e|\u641c\u7d22).*(\u5907\u5fd8|\u8bb0\u5f55)/;
const memoCreatePattern = /^(?:\u8bf7)?(?:\u5e2e\u6211)?(?:\u8bb0\u4e00\u4e0b|\u8bb0\u4f4f|\u5907\u5fd8)[\uff1a:：,\uff0c\s]*(.+)$/;
...
export async function runAgent(input: AgentInput, runtime: AgentRuntime = {}): Promise<AgentOutput> {
  ...
  if (route === "memo_create" || route === "memo_list" || route === "memo_search" || route === "memo_delete") {
...
async function handleMemoRoute(
  ...
  try {
    if (route === "memo_create") {
      ...
      if (!content) {
        return { route, reply: "要记下什么内容？例如：记一下：护照放在书桌抽屉。" };
      }
    ...
    console.error("Memo operation failed.", { error: toSafeErrorMessage(error) });
    return { route, reply: "备忘录暂时无法使用，请确认 Supabase 已创建 memos 表后再试。" };
  }
}

function extractMemoQuery(text: string): string {
  const placedItem = text.match(/(?:\u6211)?(?:\u4e4b\u524d|\u4ee5\u524d)?(?:\u628a)?(.+?)\u653e(?:\u5728|\u5230|\u54ea)/)?.[1]?.trim();
  ...
  return text
    .replace(/(我|你)?(之前|以前)?(把|记得|记录|记下|备忘|查找|搜索|帮我|一下|吗|呢|哪儿|哪里|什么地方)/g, "")
...
function formatMemos(memos: Array<{ content: string }>): string {
  return memos.slice(0, 10).map((memo, index) => `${index + 1}. ${memo.content}`).join("\n");
}
...
async function handleWeatherRoute(input: AgentInput, runtime: AgentRuntime): Promise<AgentOutput> {
  ...
  try {
    ...
    if (requestedLocation) {
      ...
      if (runtime.userPreferenceStore) {
        try {
          await runtime.userPreferenceStore.saveWeatherLocation(input.userId, location);
        } catch (error) {
      ...
      try {
        location = await runtime.userPreferenceStore.getWeatherLocation(input.userId);
      } catch (error) {
    ...
    const forecast = await runtime.weatherClient.getDailyForecast(location, target.date);
...
async function handleSearchRoute(input: AgentInput, runtime: AgentRuntime): Promise<AgentOutput> {
  ...
  try {
    const results = await runtime.searchClient.search(query, resolveSearchTopic(input.text));
...
function formatSearchSources(results: Array<{ title: string; url: string }>): string {
  return results.map((result, index) => `[${index + 1}] ${result.title}\n${result.url}`).join("\n");
}

function buildWeatherAdvice(rain: number, min: number, max: number, wind: number): string {
  const advice: string[] = [];
  ...
  return advice.length ? `${advice.join("；")}。` : "天气较平稳，正常安排出行即可。";
}
